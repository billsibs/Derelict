...
print('\n')
#print('--------------')
orderednight = nightfallStrikes.objects.order_by('teamScore').reverse().values('matchId').distinct()
#print(orderednight[:5])

standing = 1
header='{:4}|{:8}|{:25}|{:51}|'.format('No.',' Score',' Nightfall',' Guardians')
print(header)
for night in orderednight[:25]:
    guards = []
    nid = night['matchId']
    ...
    print(f"{standing:4}|{score:8}|{name:25}| {'  '.join(guards):50}|")
    standing += 1
